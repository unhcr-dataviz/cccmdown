...
Note that you will need to serve the output using a web server if using `self_contained: false`
due to [browser security policy](https://developer.mozilla.org/en-US/docs/Web/Security/Same-origin_policy#file_origins).
The easiest way to do that during local development is to call `servr::httd()` on the output directory.
